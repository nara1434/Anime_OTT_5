...
const AccountSettings = () => {
  ...
  const validateForm = () => {
    ...
    if (!validatePassword(password)) {
      setPasswordError('Password must be at least 8 characters with 1 capital letter, 1 number, and 1 special character');
      valid = false;
    }
  ...
  return (
    <div style={styles.container}>
      <form onSubmit={handleSubmit} style={styles.formBox}>
        ...
        <div style={styles.buttonGroup}>
          <button type="submit" style={{ ...styles.button, ...styles.saveButton }}>Save</button>
          <button type="button" style={{ ...styles.button, ...styles.cancelButton }} onClick={() => navigate(-1)}>Cancel</button>
        </div>
      </form>
    </div>
  );
