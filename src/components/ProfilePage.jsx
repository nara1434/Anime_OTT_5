...
import React, { useState, useEffect, useCallback } from 'react';
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
import { faPlus, faPencilAlt, faTrashAlt, faSave, faTimes } from '@fortawesome/free-solid-svg-icons';
import { Link } from 'react-router-dom';
import './ProfilePage.scss';
import NavBar from './NavBar';
...
function ProfilePage() {
  ...
  const loadProfiles = useCallback(() => {
    const profileCount = parseInt(localStorage.getItem('profileCount') || '0', 10);
    let loaded = Array.from({ length: profileCount }, (_, i) => {
      const profileData = localStorage.getItem(`profile_${i + 1}`);
      return profileData ? JSON.parse(profileData) : null;
    }).filter(Boolean);
 
    // ✅ If no profiles found, create default "Guest" profile
    if (loaded.length === 0) {
      loaded = [{ newName: 'Guest' }];
      saveProfiles(loaded);
    }
      ...
      <div className="profile-page">
        <div className="profile-management">
          ...
          <div className="profile-list">
            ...
            <Link to="/addprofile" className="add-profile-button">
              <FontAwesomeIcon className="ms-2" icon={faPlus} />
              <span style={{ marginLeft: '8px' }}>Add Profile</span>
            </Link>
          </div>
        </div>
