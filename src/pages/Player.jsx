...
const Player = () => {
  ...
  const warnings = [
    {
      time: 5,
      message: "⚠️ Trigger Warning: Emotional Abuse",
      type: "emotional",
    },
  ...
  const changeVideoQuality = (quality) => {
    ...
    const qualityMap = {
      "1080p":
        "/assets/yt1z.net - Justin Bieber - Baby ft. Ludacris (1080p).mp4",
          ...
          >
            {showThumbnail && (
              <div className="thumbnail-overlay">
                ...
                <button onClick={() => setShowThumbnail(false)}>
                  Start Watching
                </button>
              </div>
            )}
            ...
            <div className="video-overlay-buttons">
              <button
                className="seek-btn inside left"
                onClick={() => (videoRef.current.currentTime -= 10)}
              >
                ⏪
              </button>
              <button
                className="seek-btn inside right"
                onClick={() => (videoRef.current.currentTime += 10)}
              >
                ⏩
              </button>
          ...
          <div className="scene-thumbnails">
            {sceneThumbnails.map((scene, index) => (
              ...
              >
                ...
                {sceneHovered === scene.title && (
                  <div className="scene-tooltip">{scene.title}</div>
                )}
              </div>
            ))}
          </div>
